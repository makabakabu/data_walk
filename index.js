const every = (obj, func) =>
  typeOf(obj) === "array"
    ? obj.every(func)
    : Object.entries(obj).every(([key, value]) => func(value, key));
const hasOwnProperty = (obj1, obj2) =>
  typeOf(obj1) === "object" && Object.prototype.hasOwnProperty.call(obj1, obj2);
const reduce = (obj, func, initialResult) =>
  typeOf(obj) === "array"
    ? obj.reduce(func)
    : Object.entries(obj).reduce(
        (tempResult, [key, value]) => func(tempResult, value, key),
        initialResult
      );
const remove = (path, object) => {
  ...
  if (typeOf(lastObject) === "object") {
    ...
    restObject = [
      ...lastObject.slice(0, lastPath),
      ...lastObject.slice(lastPath + 1)
    ];
  }
  ...
  switch (typeOf(defaultStructure)) {
    case "string": {
      ...
      const isTypeValue = [
        ...
        "array",
        "object"
      ].includes(defaultStructure);
      if (
        (isTypeValue && typeOf(structure) === defaultStructure) ||
        (!isTypeValue && typeOf(structure) === "string")
      ) {
        return structure;
      } else {
    ...
    case "array":
      switch (
        `${typeOf(structure) ===
          typeOf(defaultStructure)} ${structure.length ===
          defaultStructure.length}`
      ) {
        case "true true":
          return defaultStructure.map((certainDefaultStructure, index) =>
            frozen(certainDefaultStructure, structure[index])
          );
    ...
    case "object":
      switch (true) {
        case hasOwnProperty(defaultStructure, "__optional__") &&
          typeOf(structure) !== "undefined" /*?*/:
          // if don't have any other property remove use body, but if we has other property then remove it.
          if (
            ["__oneOf__", "__useTypeAsValue__", "__listFunction__"].some(
              (certainProperty, index) =>
                hasOwnProperty(defaultStructure, certainProperty)
            )
          ) {
        ...
        case hasOwnProperty(defaultStructure, "__oneOf__"): {
          if (
            typeOf(defaultStructure["body"]) !== "array" ||
            defaultStructure["body"]["length"] === 0
          ) {
            console.error(
              "API接口: oneOf 类型的 body 必须为 array 类型, 而且必须有值"
            );
            return undefined;
          }
          const index = defaultStructure["body"].findIndex(
            (certainDefaultStructure, index) =>
              typeEqual(certainDefaultStructure, structure)
          ); /*?*/
          switch (true) {
            case index === -1 &&
              hasOwnProperty(defaultStructure, "__defaultIndex__"):
              return defaultStructure["body"][
                defaultStructure["__defaultIndex__"]
              ];
            case index === -1 &&
              !hasOwnProperty(defaultStructure, "__defaultIndex__"):
              return undefined;
        ...
        default:
          return reduce(
            defaultStructure,
            (tempResult, certainDefaultStructure, key) => {
              switch (true) {
                case hasOwnProperty(certainDefaultStructure, "__optional__") &&
                  !hasOwnProperty(structure, key):
                  return tempResult;

                case !hasOwnProperty(structure, key):
                  return {
                    ...tempResult,
                    [key]: frozen(certainDefaultStructure)
                  };

                case hasOwnProperty(structure, key):
                  return {
                    ...tempResult,
                    [key]: frozen(certainDefaultStructure, structure[key])
                  };
...
const typeEqual = (defaultStructure, structure) => {
  ...
  switch (typeOf(defaultStructure)) {
    ...
    case "array":
      if (
        `${typeOf(defaultStructure) ===
          typeOf(structure)} ${defaultStructure.length === structure.length}`
      ) {
        return defaultStructure.every((certainDefaultStructure, index) =>
          typeEqual(certainDefaultStructure, structure[index])
        );
      } else {
        return false;
      }
    ...
    case "object":
      switch (true) {
        ...
        case hasOwnProperty(defaultStructure, "__oneOf__"):
          return (
            defaultStructure.body.findIndex((certainDefaultStructure, index) =>
              typeEqual(certainDefaultStructure, structure)
            ) !== -1
          );
...
Symbol("aaa").toString() === Symbol("aaa").toString(); /*?*/
Symbol("Symbol(aaa)"); /*?*/

// should also export a collection of the Symbol represent some remain word like Symbol
// if you really want to use symbol:
